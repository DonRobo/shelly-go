...
func EmitClient(spec *Spec, outDir string) error {
	...
	// hand-curated list. Gap-filled like the rest: skipped while a hand-written
	// aggregate still exists.
	if !handConfig["shelly"] && !handStatus["shelly"] {
		agg, err := emitAggregates(spec)
		if err != nil {
			return fmt.Errorf("emit aggregate: %w", err)
		}
		if err := os.WriteFile(filepath.Join(outDir, "shelly_gen.go"), agg, 0o644); err != nil { //nolint:gosec // generated source.
			return err
		}
	}
	return nil
